...
parsedSamplePoints = map(lambda l: parsePoint(l),samplePoints)
...
# TODO: Replace <FILL IN> with appropriate code
parsedDataInit = rawData.map(lambda l: parsePoint(l))
onlyLabels = parsedDataInit.map(lambda l: l.label)
...
# TODO: Replace <FILL IN> with appropriate code
parsedData = parsedDataInit.map(lambda l: LabeledPoint(l.label-1922,l.features))
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return np.sqrt(labelsAndPreds.map(lambda l: squaredError(l[0],l[1])).mean())
...
# TODO: Replace <FILL IN> with appropriate code
labelsAndPredsTrain = parsedTrainData.map(lambda l: (l.label,averageTrainYear))
rmseTrainBase = calcRMSE(labelsAndPredsTrain)

labelsAndPredsVal = parsedValData.map(lambda l: (l.label,averageTrainYear))
rmseValBase = calcRMSE(labelsAndPredsVal)

labelsAndPredsTest = parsedTestData.map(lambda l: (l.label,averageTrainYear))
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        gradient = trainData.map(lambda l: gradientSummand(DenseVector(w),l)).sum()
...
labelsAndPreds = parsedValData.map(lambda l: getLabeledPrediction(weightsLR0,l))
...
# TODO: Replace <FILL IN> with appropriate code
labelsAndPreds = parsedValData.map(lambda l: (l.label,firstModel.predict(l.features)))
...
def twoWayInteractions(lp):
    ...
    """
    return LabeledPoint(lp.label,np.hstack((lp.features,DenseVector(map(lambda l: l[0]*l[1],list(itertools.product(lp.features,repeat=2)))))))

print twoWayInteractions(LabeledPoint(0.0, [2, 3]))

# Transform the existing train, validation, and test sets to include two-way interactions.
trainDataInteract = parsedTrainData.map(lambda l: twoWayInteractions(l))
valDataInteract = parsedValData.map(lambda l: twoWayInteractions(l))
testDataInteract = parsedTestData.map(lambda l: twoWayInteractions(l))
