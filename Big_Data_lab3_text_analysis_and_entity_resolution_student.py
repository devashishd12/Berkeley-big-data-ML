...
def simpleTokenize(string):
    ...
    """
    return map(lambda s: s.lower(),filter(None,re.split('\W+',string)))
...
def tokenize(string):
    ...
    """
    return filter(lambda s: s if s not in stopwords else '',map(lambda s: s.lower(),filter(None,re.split('\W+',string))))
...
def countTokens(vendorRDD):
    ...
    """
    return (vendorRDD.map(lambda l: len(l[1])).reduce(lambda a,b:a+b))
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.takeOrdered(1,lambda l: -1*len(l[1]))
...
def tf(tokens):
    ...
    """
    dict={l: tokens.count(l)/float(len(tokens)) for l in tokens}
    return dict
...
# TODO: Replace <FILL IN> with appropriate code
corpusRDD = amazonRecToToken.map(lambda l: (l[0],l[1])) + googleRecToToken.map(lambda l: (l[0],l[1]))
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.flatMap(lambda l: list(set(l[1])))
    tokenCountPairTuple = uniqueTokens.map(lambda l: (l,1))
    tokenSumPairTuple = tokenCountPairTuple.reduceByKey(lambda a,b: a+b)
    return (tokenSumPairTuple.map(lambda (x,y): (x,float(N)/y)))
...
def tfidf(tokens, idfs):
    ...
    """
    tfs = tf(tokens)
    tfIdfDict = {k : v * tfs[k] for k, v in idfs.items() if k in tfs}
    return tfIdfDict
...
similarities = (crossSmall
                .map(lambda l: computeSimilarity(l))
                .cache())
...
similaritiesBroadcast = (crossSmall
                         .map(lambda l:computeSimilarityBroadcast(l))
                         .cache())
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda l: (l[1]+" "+l[0],l[2]))

trueDupsRDD = (sims.join(goldStandard).map(lambda l: l[1][0]))
trueDupsCount = trueDupsRDD.count()
avgSimDups = trueDupsRDD.reduce(lambda a,b:a+b)/float(trueDupsCount)

nonDupsRDD = (sims
              .subtractByKey(goldStandard).map(lambda l: l[1]))
...
# TODO: Replace <FILL IN> with appropriate code
amazonFullRecToToken = amazon.map(lambda l: (l[0],tokenize(l[1])))
googleFullRecToToken = google.map(lambda l: (l[0],tokenize(l[1])))
...
# TODO: Replace <FILL IN> with appropriate code
fullCorpusRDD = amazonFullRecToToken.map(lambda l: (l[0],l[1])) + googleFullRecToToken.map(lambda l: (l[0],l[1]))
...
amazonWeightsRDD = amazonFullRecToToken.map(lambda l: (l[0],tfidf(l[1],idfsFullBroadcast.value)))
googleWeightsRDD = googleFullRecToToken.map(lambda l: (l[0],tfidf(l[1],idfsFullBroadcast.value)))
...
commonTokens = (amazonInvPairsRDD
                .join(googleInvPairsRDD).map(lambda l: swap(l)).groupByKey()
                .cache())
...
similaritiesFullRDD = (commonTokens
                       .map(lambda l: fastCosineSimilarity(l))
                       .cache())
