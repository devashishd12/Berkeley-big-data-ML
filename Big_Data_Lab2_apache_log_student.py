...
hosts = access_logs.map(lambda logs: logs.host)
...
avgDailyReqPerHost = (sortedByDay
                      .join(dailyHosts).map(lambda (a,(b,c)):(a,b/c)).sortByKey().cache())
...
daysWithAvg = avgDailyReqPerHost.map(lambda (a,b): a).collect()
avgs = avgDailyReqPerHost.map(lambda (a,b): b).collect()
...
errDateCountPairTuple = badRecords.map(lambda l: (l.date_time.day,1))
...
daysWithErrors404 = errDateSorted.map(lambda (a,b): a).collect()
errors404ByDay = errDateSorted.map(lambda (a,b): b).collect()
