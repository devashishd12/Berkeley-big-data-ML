...
movieIDsWithRatingsRDD = (ratingsRDD
                          .map(lambda l: (l[1],l[2])).groupByKey())
...
movieIDsWithAvgRatingsRDD = movieIDsWithRatingsRDD.map(lambda l: getCountsAndAverages(l))
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              .join(movieIDsWithAvgRatingsRDD).map(lambda l: (l[1][1][1],l[1][0],l[1][1][0])))
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD).map(lambda l: (l[0],math.pow(l[1][0]-l[1][1],2))))

    # Compute the total squared error - do not use collect()
    totalError = squaredErrorsRDD.map(lambda l: l[1]).sum()
...
trainingAvgRating = trainingRDD.map(lambda l: l[2]).mean()
print 'The average rating for movies in the training set is %s' % trainingAvgRating

testForAvgRDD = testRDD.map(lambda l: (l[0],l[1],trainingAvgRating))
...
myRatedMovies = [
     (myUserID, 1704, 3),(myUserID, 2692, 3),(myUserID, 318, 5),(myUserID, 1213, 5),(myUserID, 527, 5),(myUserID, 2571, 5),(myUserID, 296, 5),(myUserID, 1240, 3),(myUserID, 1, 4),(myUserID, 858, 5)
...
myUnratedMoviesRDD = (moviesRDD
                      .map(lambda l: (myUserID,l[0])).filter(lambda l: l[1] not in [i[1] for i in myRatedMovies]))
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda l: (l[0],l[1][0]))

# Transform predictedRatingsRDD into an RDD with entries that are pairs of the form (Movie ID, Predicted Rating)
predictedRDD = predictedRatingsRDD.map(lambda l: (l[1],l[2]))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD).map(lambda l:(l[1][0][0],l[1][1],l[1][0][1])).filter(lambda l: l[2]>75))
